...
export const makeTools = (store: SessionStore, client: PiClient) => {
  const tools = {
    ...
    pi_get_result: async ({ session_id }: SessionIdParam) => {
      const entry = store.get(session_id)
      if (!entry) throw { kind: 'session_not_found' as const, session_id } satisfies PiError
      if (entry.status === 'error') throw { kind: 'pi_unavailable' as const, message: entry.error ?? 'unknown' } satisfies PiError
      ...
      return new Promise<{ output: string }>((resolve, reject) => {
        const interval = setInterval(() => {
          ...
          if (!e || e.status === 'error') {
            ...
            reject({ kind: 'pi_unavailable' as const, message: e?.error ?? 'session lost' } satisfies PiError)
